...
_install_in_container(){
    app=$1
    $(podman exec -t deepin dpkg -L $app > /dev/null 2>&1)
    if [[ $? -ne 0 ]]; then
        #install
        echo "Install to container..."
        podman exec -u 0 deepin apt install -y ${app}
        return $?
    fi
    return 0
}
...
_install_desktop(){
    app=$1
    icons=$(podman exec -t deepin sh -c "IFS=$'\r\n' dpkg -L ${app} | grep -E /icons/.+")
    IFS=$'\r\n'
    # install icons
    for i in $icons; do
        # id=`podman inspect -f   '{{.Id}}' deepin`
        from=$i
        [ -d $i ] && continue
        to=$HOME/.local/share/icons/$(echo $from | sed -n -r -e 's/^.+icons\/(.+)$/\1/p')
        echo "deepin:$from -> $to"
        mkdir -p `dirname $to`
        podman cp "deepin:$from" "$to"
        # convert svg to png
        [[ "${to##*.}" == "svg" ]] && {
            echo "convert to:"
            echo "${to%.*}.png"
...
EOF
    ...
    podman exec -u deepin -t deepin sh -c "$sc"
...
run(){
    ...
    case $key in
        deepin.com.thunderspeed|deepin.com.taobao.wangwang|deepin.com.taobao.aliclient.qianniu|deepin.com.qq.rtx2015|deepin.com.qq.office|deepin.com.qq.im.light|deepin.com.qq.im|deepin.com.qq.b.eim|deepin.com.qq.b.crm|deepin.com.gtja.fuyi|deepin.com.foxmail|deepin.com.cmbchina|deepin.com.baidu.pan|deepin.com.aaa-logo|deepin.com.95579.cjsc|deepin.cn.com.winrar|deepin.cn.360.yasuo|deepin.com.wechat|deepin.com.weixin.work|deepin.net.263.em|deepin.org.7-zip|deepin.org.foobar2000|deepin.net.cnki.cajviewer)
            ...
            echo "exec command:"
            echo "podman exec -u deepin -d deepin $cmd"
            podman exec -u deepin -d deepin "$cmd"
        ;;
        *)
            echo "Unknown opt."
            exit 1
...
help(){
    echo "Usage: podman-deepin COMMAND"
    echo ""
    echo "Commands:"
    echo ""
    echo "--init|init"
...
case $cmd in
    ...
    ;;
    --install|install)
        timeout 600 podman exec -u 0 deepin sh -c "apt update"
        l=(${@})
        for app in ${l[@]:1} ; do
            echo "install $app"
            install $app
